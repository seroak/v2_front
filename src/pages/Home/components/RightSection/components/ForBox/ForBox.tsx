    ...
    <AnimatePresence key={forItem.id}>
      <motion.div className={styles.for}>
        ...
        >
          ...
          <motion.div layout className={cx(styles['cur-number'], forItem.isLight && forItem.isCurLight && styles['highlight-number'])}>
            <span className="white-text">{forItem.cur}</span>
          </motion.div>
          <motion.div layout className={styles['start_title']}>
            <span>start</span>
          </motion.div>
          <motion.div layout className={cx(styles.number_start, forItem.isLight && forItem.isStartLight && styles['highlight-number'])}>
          ...
          <motion.div layout className={cx(styles.end_number, forItem.isLight && forItem.isEndLight && styles['highlight-number'])}>
          ...
          {forItem.step === '1' ? null : (
            <>
              ...
              <motion.div layout className={cx(styles.step_number, forItem.isLight && forItem.isStepLight && styles['highlight-number'])}>
          ...
          {children && (
            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: 0.3 }}>
              {children}
            </motion.div>
          )}
        </motion.div>
